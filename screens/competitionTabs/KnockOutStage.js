...
import { fetchKnockOutStage } from "../../store/actions/edition";
const KnockOutStage = (props) => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchKnockOutStage(id)).then(() => {
  ...
  //  Go to next stage
  const goToNextStage = () => {
    if (selectedStage == props.knockoutstage.length - 1) setSelectedStage(0);
    else setSelectedStage(selectedStage + 1);
  };
  //  Go to previous stage
  const goToPreviousStage = () => {
    if (selectedStage == 0) setSelectedStage(props.knockoutstage.length - 1);
    else setSelectedStage(selectedStage - 1);
  };
  ...
  const renderSummary = () => {
    let stats = props.knockoutstage[selectedStage].stats;
    ...
    <NativeBaseProvider>
      ...
      >
        <Row style={Styles.header}>
          ...
          >
            {props.knockoutstage.map((stage, index) => {
              return (
                <Select.Item
        ...
        <ScrollView>
          {props.knockoutstage[selectedStage].games &&
            props.knockoutstage[selectedStage].games.map((match, index) => {
...
const mapStateToProps = (state) => {
  return {
    knockoutstage: state.edition.knockoutstage,
  };
};
export default connect(mapStateToProps)(KnockOutStage);
